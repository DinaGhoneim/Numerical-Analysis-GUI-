...
def false_position():
    ...
    label = CTkLabel(master=window, text="False Position Method", font=("Times New Roman", 50), text_color="#00246B",
                     fg_color="#CADCFC")
    ...
    def insert_data():
        ...
        if switch.get() == 1:
            et = int(et_entry.get())
            while i <= et:
                xr = float(xu-((func.subs(x, xu)*(xl-xu))/(func.subs(x, xl)-func.subs(x, xu))))
                if i == 1:
                    eerror = '-----'
                input_data = [
                    i,
