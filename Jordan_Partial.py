...
def Jordan_Partial():
    ...
    def gauss_e():
            ...
            for i in range(n):
                ...
                for j in range(n):
                    if i != j:
                        ...
                        it = display_matrix(matrix, f"After Using:{factor}R{i + 1} - R{j + 1}->R{j + 1}")
        ...
        window.title("Gauss Jordan with Partial Pivoting")
        frames = []
        for i in range(rows):
    ...
    window.title("Gauss Jordan with Partial Pivoting")
    label1 = CTkLabel(master=window, text="Gauss With Partial Pivoting", font=("Times New Roman", 50),
